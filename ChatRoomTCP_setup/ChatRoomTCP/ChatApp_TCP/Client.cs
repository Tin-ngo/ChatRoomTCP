       ...
       // List<Socket> ClientList;   //danh sách lưu các client - nhận từ server
        ...
        public int Connect()
        {
            ...
            try
            {
                ...
                // ổ cắm ở phía server
                client = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp); // mặc định là phải ghi như vậy
                    ...
                    try
                    {
                        ...
                        if (obj.GetType().ToString() == "System.String")
                        {
                            ...
                            try
                            {
                                ...
                                String[] name_client = message.Split(characters);//==tên || GuiRieng==mess
